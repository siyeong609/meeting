...
/**
 * /admin/rooms/create
 * - POST: room 생성(운영시간 7일 포함)
 * - 요청은 FormData(멀티파트X, 단순 폼) 또는 x-www-form-urlencoded 기준으로 받는다.
 */
@WebServlet(urlPatterns = "/admin/rooms/create")
public class AdminRoomCreateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            RoomDetail room = parseRoomFromParams(req, false);

            int newId = roomService.create(room);

            Map<String, Object> data = new HashMap<>();
            data.put("id", newId);

            out.put("ok", true);
            out.put("message", "생성 완료");
            out.put("data", data);
            out.put("page", null);
