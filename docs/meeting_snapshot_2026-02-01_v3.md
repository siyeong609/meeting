...
│  │  └─ profile.css                 # 내정보수정 전용 레이아웃(그리드/버튼 정렬)
...
│  │  ├─ profile.js                  # 내정보수정(이메일/비번/프로필 업로드/삭제)
│  │  └─ room
│  │     ├─ list.js                  # 회의실 목록 + 예약 현황(일자/달력)
│  │     └─ detail.js                # 회의실 상세 + 예약 현황(일자/달력)
│  └─ admin
│     ├─ member
...
### 13.4 내정보수정(회원 프로필) ✅ (최근 수정 포함)
- `UserProfileServlet` / `UserProfileUpdateServlet` / `UserProfileUploadServlet`
- 프로필 이미지 정책: 커스텀 업로드만 DB 저장, 없으면 NULL + default 이미지 표시
- ✅ 레이아웃 적용: `profile.jsp`는 header/footer 레이아웃 포함 후 컨텐츠만 렌더링
- ✅ 중요 버그 수정 포인트:
  - `profile.js`가 로드될 때 `window.__MEETING__.ctx`가 아직 없으면 `ctx=""`로 고정됨
  - 해결: `profile.jsp`에서 **profile.js 로드 전에 ctx를 선주입**(footer.jsp의 ctx 주입은 유지해도 됨)

### 13.5 회의실 예약 현황 UI(회원) ✅/진행중
- `user/room/list.js`: 목록 + 하단 “예약 현황” 카드(탭: 일자/달력)
- `user/room/detail.js`: 상세 + “예약 현황” 카드(탭: 일자/달력)
- ✅ UX 수정:
  - `tabMonth` 클릭 시 즉시 월 현황 로드(버튼 클릭 없이)
  - 달력 렌더 함수 통합 방향: `calendarWrap` 기준으로 `renderMonthCalendar()` 스타일로 정리 중
- 🔧 이슈/정리(진행중):
  - day 타임라인 렌더: 예약 배열(reservations) 존재하나 슬롯 겹침 표시 정합성/마크업 닫힘 여부 점검 필요

### 13.6 1:1 채팅 위젯(MVP) ✅/진행중
...
- [ ] 회의실 예약 현황(일/월): 렌더/UI 통합 마무리(탭 자동로드, 달력 renderer 통일, 타임라인 겹침 표시 확정)
...
- 회원 프로필(내정보수정) 페이지 레이아웃 적용(profile.css, room-card 스타일 재사용)
- 회원 프로필 저장 불가 이슈 원인 확정 및 수정 포인트 정리(✅ profile.js 로드 전 ctx 선주입)
- 회의실 예약 현황(일/월) UI 통합 작업 진행(tabMonth 즉시 로드, 달력 renderer 통일 방향)
...
- 🔧 버퍼 정책 정리 필요(사용자가 기대하는 “13~14 예약 가능” 케이스 기준으로 buffer 적용 범위 확정)
...
## 17. 다음 대화에서 시작 문구(추천)
- `meeting 스냅샷 v2 기준으로 (1) profile.jsp ctx 선주입 반영 여부 확인하고, (2) 예약 현황 달력 renderer 통일 + tabMonth 즉시 로드까지 마무리하자`
